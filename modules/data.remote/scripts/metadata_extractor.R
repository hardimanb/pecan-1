...
metadata_extractor<-function(metadata.inpath){
  ...
  for(n in 1:numfiles){
    data.path<-file.path(inpath,list.files(inpath)[n])
    
    ##Parameters I (Metadata) will be extracted from these files:
    workreport <- read.delim(file.path(data.path,"workreport"), header=F, sep="=", fill=T)
    
    ##scene ID
    scnid   <- as.character(workreport$V2[grep("Scs_SceneID",workreport$V1)])
    
    ##Parameters II (Metadata) will be extracted from these files:
    txtfile <- read.delim(file.path(data.path, paste(scnid,".LED.txt",sep="")),header=F, sep="\t", strip.white=T,fill=T,blank.lines.skip=T)

    ##Incidence angle
    angle <- as.numeric(as.character(txtfile[grep("INCIDENCE ANGLE", txtfile$V1),length(txtfile[grep("INCIDENCE ANGLE", txtfile$V1),])]))
    ...
    if(n==1){
      ...
      metadata<-rbind(metadata, scn_vector)
    }
  
  } #FOR loop (looping over palsar scenes)
  
  colnames<-c("scnid","scndate","angle","scnUTMzone", "scnpix","scncf","scn_pix2coord_a11","scn_pix2coord_a12",
              "scn_pix2coord_a13","scn_pix2coord_a14","scn_pix2coord_a21","scn_pix2coord_a22",
              "scn_pix2coord_a23","scn_pix2coord_a24","scn_coord2pix_b11","scn_coord2pix_b12","scn_coord2pix_b13",
              "scn_coord2pix_b14","scn_coord2pix_b21","scn_coord2pix_b22","scn_coord2pix_b23","scn_coord2pix_b24",
              "scn_coord2pix_a0","scn_coord2pix_a1","scn_coord2pix_a2","scn_coord2pix_a3","scn_coord2pix_a4",
              "scn_coord2pix_a5","scn_coord2pix_a6","scn_coord2pix_a7","scn_coord2pix_a8","scn_coord2pix_a9",
              "scn_coord2pix_b0","scn_coord2pix_b1","scn_coord2pix_b2","scn_coord2pix_b3","scn_coord2pix_b4",
              "scn_coord2pix_b5","scn_coord2pix_b6","scn_coord2pix_b7","scn_coord2pix_b8","scn_coord2pix_b9",
              "scn_nwlat","scn_nwlon","scn_nelat","scn_nelon","scn_swlat","scn_swlon","scn_selat","scn_selon",
              "scn_centlat",
              "scn_centlon")  
